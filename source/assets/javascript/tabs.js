;(function($, window) {
    function Tabs(element, config) {
        ...
        if(this.config.openFirst) {
            $(this.element).find(this.config.children).removeClass('is-active').addClass('is-inactive');
            var $child = $(this.element).find(this.config.children).first();

            $child.addClass('is-active').removeClass('is-inactive');

            if($child.attr('id')) {
                $('[' + this.config.attribute + '=\'#' + $child.attr('id') + '\']').addClass('is-current');
            }
        }
    }
    ...
    Tabs.prototype.bindEvents = function() {
        ...
        $(this.element).find(this.config.children).each(function() {
            if($(this).attr('id')) {
                var target = $('[' + self.config.attribute + '=\'#' + $(this).attr('id') + '\']');

                target.on('click', function(event) {
                    event.preventDefault();

                    $(target).parent().siblings().find('a').removeClass('is-current');
                    $(target).addClass('is-current');

                    if(!self.config.allowMultiple) {
                        $(self.element).find(self.config.children).removeClass('is-active').addClass('is-inactive');
                        ...
                        var activeChildren = $(self.element).find(self.config.children + '.is-active');
                        ...
                        if(!$(target).hasClass('is-active')) {
                            $(target).toggleClass('is-active').toggleClass('is-inactive');
                        }
    ...
    function Filters(element, config) {
        ...
        $(this.element).find(this.config.children).each(function() {
            if($(this).attr('id')) {
                $('[' + self.config.attribute + '=\'#' + $(this).attr('id') + '\']').on('click', function(event) {
                    this.siblings().removeClass('is-active').addClass('is-inactive');
                }.bind($(this)));
            }
        });
    }
